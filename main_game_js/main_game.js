...
var completeImage = new Uint32Array(ctx.getImageData(0, 0, 1920, 1080).data.buffer);
...
function Car(x = 262, y = 165) 
{
    this.x = x;
    this.y = y;
    this.velocity = new vector(1 + rand(10)/10, [-1, 1][rand(1)] * rand(10)/10);
    this.state = 'start';
    this.maxSpeed = 3.2;
    ...
    for (let i = 0; i < this.numberOfCamers; i++) 
    {
        this
            .cameras
            .push(new camera(this.x, this.y, this.r + this.cameraDivergence * (i / this.numberOfCamers - 1 / 2), this.cameraRange));
        this
            .cameraData
            .push(0);
    }
    ...
    this.update = () => 
    {
        if (!this.isDead)
        {
            ...
            if (this.velocity.magnitude < this.maxSpeed) 
            {
                this.velocity = this
                    .velocity
                    .add(this.velocity.unitVector().intensify(this.acceletation));
                this.state = 'start';
                ...
                this.velocity = this
                    .velocity
                    .unitVector()
                    .intensify(this.maxSpeed);
            }

            if (this.steerData[0] && this.velocity.mod != 0)
            {
                if (this.velocity.mod < this.velocity.unitVector().intensify(this.breakingConstant).mod) 
                {
                    this.state = 'stop';
                }
                else 
                {
                    this.velocity = this
                        .velocity
                        .sub(this.velocity.unitVector().intensify(this.breakingConstant));
                }
            }
            ...
            if (this.steerData[1])
            {
                this.velocity = this
                    .velocity
                    .add(this.velocity.perp().intensify(-1 * this.steerData[1] * this.steeringConstant));
            }
            ...
            if (this.steerData[2]) 
            {
                this.velocity = this
                    .velocity
                    .add(this.velocity.perp().intensify(this.steerData[2] * this.steeringConstant));
            }
            ...
            if ((Math.abs(this.x - this.lastX) < 0.8) && (Math.abs(this.y - this.lastY) < 0.8)) {this.notMovedFrame++;}
            else {this.notMovedFrame=0;}
...
function camera(carX, carY, degree, range) 
{
    ...
    this.draw = () => 
    {
        ctx.fillStyle = '#fff';
        ctx.beginPath();
        ctx.arc(this.range * Math.cos(this.degree), this.range * Math.sin(this.degree), 1, 0, Math.PI * 2);
        ctx.fill();
...
function run()
{
    ...
    {
        fitnesses = [];
        for (let idx = 0; idx < carList.length; idx++) { fitnesses.push(carList[idx].calculateFitness()); }
    ...
    else
    {
        for (carIndex in carList)
        {
            ...
            {   
                ...
                if (!thisCar.isDead) {cameraData.push(thisCar.cameraData); thisCar.update();}
...
{
    isDeads = 0;
    for (let idx = 0; idx < carList.length; idx++) { if (carList[idx].isDead) {isDeads += 1;} }
...
function getControlData (cameraData)
{
    ...
    for (let idx = 0; idx < genetic_algo.population.length; idx++) 
    {
        if (cameraData[idx] != 0) 
        {
            ...
            (data[1] > data[2])? (data[1] = 1, data[2] = 0): (data[1] = 0, data[2] = 1);
        }
        else {data = [0, 0, 0];}
...
function CountLiveCars () {return carList.filter((car) => !Boolean(car.isDead)).length;}

function showLegend(fps) {
    document
        .getElementById('fps')
        .innerText = 'FamePerSec ' + fps;

    document
            .getElementById('gen')
            .innerHTML = 'Generation: ' + genetic_algo.generations;
    
    document
            .getElementById('pop')
            .innerHTML = 'Population: ' + live_cars;
}

function drawBanner(timeTaken) {
    document
        .getElementById('time')
        .innerText = 'time taken : ' + timeTaken + ' seconds';
    document
